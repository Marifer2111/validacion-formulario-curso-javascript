...
function validarFormulario(){
    ...
    camposTexto.forEach(campo => {
        let errorCampo= document.getElementById('error' + campo.id.charAt(0).toUpperCase() + campo.id.slice(1)) // error + id con la primera letra en mayùscula
    ...
    if(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.value)){
        ...
        mostrarError(errorEmail, '¡Ingrese un correo electrónico válido!') //Este regax válida que el formulario del formato email sea válido
    }
   ...
   const errorNivelEstudio = document.getElementById('errorNivelEstudio')
    ...
    return validacionCorrecta // esto dirá si el formulario esta completo
}
